...
const findIDs_1 = require("../utils/findIDs");
...
async function checkUserExist(email) {
    ...
    try {
        const payload = {
            Field: "email",
            Value: email,
            MerchantId: variable_1.DOMAIN_NCC_CONSTANT.MERCHANT_ID_theinner_techcombank_NCC
...
async function buildNCCUser(item, options) {
    /**
 ...
 */
    const user = {
        ...
        CreatedBy: variable_1.DOMAIN_NCC_CONSTANT.GIANGPM_ID,
        ...(item.Code && {
            Code: item.Code
        }),
    ...
    if (options?.updateDateAddress && item.IdentifyAddress) {
        const addressId = await (0, findIDs_1.findAddressIdByName)(item.IdentifyAddress, {
